...
class CompositeModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_norm', 'G_focal', 'D_real', 'D_fake', 'D_correctness']
        # self.loss_names = ['G_GAN', 'G_norm', 'G_focal', 'D_critic', 'D_correctness']
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(self.netG.parameters(), lr=opt.lr * 10, betas=(opt.beta1, 0.999),
                                                weight_decay=0.01)
    ...
    def backward_D(self):
        ...
        pred_fake, pred_correctness = self.netD(fake_AB.detach())
        # pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        self.loss_D_correctness = self.criterionBCE(pred_correctness,
                                                    self.current_correctness.expand_as(pred_correctness))

        # dist = (realB_length - fake_length).sum().item()
        ...
        # length_real = utils.length_label(self.fake_B.cpu()).unsqueeze(-1).to(self.device)
        # self.wirelength += length_real.sum().item()
        # self.loss_D_length = self.criterionMSE(pred_length, length_real)

        # Real
        # smooth_real_B = utils.smooth(self.real_B.cpu()).to(self.device)
        # real_AB = torch.cat((self.real_A, smooth_real_B), 1)
        real_AB = torch.cat((self.real_A, self.real_B), 1)
        pred_real, pred_constraint = self.netD(real_AB)  # composite D
        # pred_real = self.netD(real_AB)    #  basic_N_layer_D
        self.loss_D_real = self.criterionGAN(pred_real, True)
        self.loss_D_correctness += self.criterionGAN(pred_constraint, True)

        # self.loss_D_critic = self.criterionD(pred_real, pred_fake)  # hinge loss

        # combine loss and calculate gradients
        self.loss_D = self.loss_D_fake + self.loss_D_real + self.loss_D_correctness
        # self.loss_D = self.loss_D_critic + self.loss_D_correctness  # hinge loss
    ...
    def backward_G(self):
        ...
        pred_fake, _ = self.netD(fake_AB)  # composite D
        # pred_fake = self.netD(fake_AB)    #  basic_N_layer_D
        self.loss_G_GAN = self.criterionGAN(pred_fake, True)
        # self.loss_G_GAN = self.criterionD(pred_fake)  # hinge loss
        # Second, G(A) = B
        ...
        self.loss_G = (self.loss_G_norm + self.loss_G_focal) * (1 + 1e-3 * (100 * np.sign(self.dist - 1) + 1) *
                                                                self.dist) + self.loss_G_GAN  # + self.loss_M_L1*0.1
        # print(self.loss_G)
